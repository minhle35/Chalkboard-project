    ...
    def get_user(user_id):
        ...
        try:
            ...
            if role in ("instructor","student"):
                courses = []
                if avatar_url:
                    avatar_url = f"{APP_URL}/users/{user_id}/avatar"
    ...
    def update_course(course_id):
        """_summary_: function to update a course based on provided information in the request body. Student enrollment cannot be modified via this endpoint. Protection: Only users with the role admin
        ...
        try:
            ...
            if "instructor_id" in provided_fields:
                instructor_id = data["instructor_id"]
                instructor_key = client.key("users", int(instructor_id))
                instructor_entity = client.get(instructor_key)
                if not instructor_entity or instructor_entity.get("role") != "instructor":
    ...
    # Requirement 12. Update enrollment in a course
    @app.route('/courses/<course_id>/students', methods=['PATCH'])
    def update_student_enrollment(course_id):
        """
        Update (enroll or disenroll) students in a course based on provided information in the request body.
        Protection: Only users with the role admin or the instructor of the course.

        Args:
            course_id (int)
        Request Body:
            {
                "add": [student_id1, student_id2],
                "remove": [student_id3]
            }
        Response:
            - 200: Success, empty body
            - 401: JWT is missing or invalid
            - 403: Unauthorized or course does not exist
            - 409: Invalid enrollment data
        """
        try:
            # Verify JWT
            payload = verify_jwt(request)
            ...
            is_admin = payload["nickname"].startswith("admin")
            course_key = client.key("courses", int(course_id))
            course_entity = client.get(course_key)
            ...
            expected_instructor_id_datastore = course_entity.get("instructor_id")

            user_key = client.key("users", int(expected_instructor_id_datastore))
            user_entity = client.get(user_key)
            expected_instructor_id = None
            if user_entity and "sub" in user_entity:
                expected_instructor_id = user_entity["sub"]
            actual_instructor_query = payload["sub"]

            if expected_instructor_id:
                if not is_admin and expected_instructor_id.strip() != actual_instructor_query.strip():
                    return jsonify({"Error": "You don't have permission on this resource"}), 403
            ...
            # Validate all IDs in "add" and "remove" correspond to students
            user_ids = list(set(add + remove))
            invalid_ids = []
            for user_id in user_ids:
                user_key = client.key("users", int(user_id))
                user_entity = client.get(user_key)
                if not user_entity or user_entity.get("role") != "student":
                    invalid_ids.append(user_id)

            if invalid_ids:
                return jsonify({"Error": "Enrollment data is invalid"}), 409

            enrolled_students = course_entity.get("student_list", [])
            if enrolled_students == ["propertyValue"]:
                enrolled_students = []


            # Process enrollments (add students)
            ...
            # Process disenrollments (remove students)
            for student_id in remove:
                if student_id in enrolled_students:
                    enrolled_students.remove(student_id)
                    print(f"Disenrolled student {student_id} from course {course_id}.")
            # Update the course entity with the modified enrolled_students list
            course_entity["enrolled_students"] = enrolled_students
            client.put(course_entity)
            return "", 200
    ...
    # Requirement 13. Get enrolled students for a course
    ...
    def get_enrolled_students(course_id):
        ...
        try:
            ...
            course_key = client.key("courses", int(course_id))
            course_entity = client.get(course_key)
            print("course_entity", course_entity)
            ...
            enrolled_students = course_entity.get("enrolled_students", [])
            return enrolled_students, 200

        except AuthError as e:
            raise

    # Requirement 11. Delete a course
    @app.route('/courses/<course_id>', methods=['DELETE'])
    def remove_course(course_id):
        """_summary_: function to remove a course
        Also deletes enrollment of all students that were enrolled in the course.
        The instructor teaching the course is no longer associated with the course.
        Protection: Only users with the role admin
